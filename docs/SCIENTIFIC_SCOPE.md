...
`multiRegFlow` is an integration and interpretation layer for multiple
regression in agricultural, soil and plant sciences. It does not attempt
to reimplement every statistical estimator. Specialized estimation is
delegated to established R packages, while `multiRegFlow` provides a
coherent workflow, stable objects, agronomic examples, validation,
...
1.  **explanation/inference**, where the goal is conditional
    association, uncertainty and scientific interpretation under a
    prespecified model; and
2.  **prediction**, where the goal is performance for observations not
    used to fit the model.

This distinction controls how selection, shrinkage and model comparison
are interpreted.
...
- optional
  ...
  output, including GVIF handling where supported.
...
- one-way cluster-robust covariance using the real sampling/experimental
...
Selection is explicitly described as exploratory unless the inferential
procedure accounts for the search.
...
The bootstrap unit must follow the design. The built-in bootstrap is
intended for independent observational units.
...
These measures are deliberately not treated as interchangeable
definitions of variable importance.

### Flexible alternatives

- robust regression through
  [`robustbase::lmrob()`](https://rdrr.io/pkg/robustbase/man/lmrob.html);
- quantile regression through
  [`quantreg::rq()`](https://rdrr.io/pkg/quantreg/man/rq.html);
- GAM through [`mgcv::gam()`](https://rdrr.io/pkg/mgcv/man/gam.html);
- kernel regression through
  [`np::npregbw()`](https://rdrr.io/pkg/np/man/np.regression.bw.html)
  and [`np::npreg()`](https://rdrr.io/pkg/np/man/np.regression.html).

These models are sensitivity analyses or alternatives motivated by a
scientific target or diagnosed functional-form problem. They are not
automatic upgrades over OLS.
...
Metrics include RMSE, MAE, out-of-fold R-squared, calibration intercept
...
The following are not part of the current core because they would change
the scientific purpose of the package:
...
These may be useful comparison or future-extension topics but should not
turn the package into a generic machine-learning framework.
...
- no use of [`attach()`](https://rdrr.io/r/base/attach.html);
...
- optional backends checked through
  [`requireNamespace()`](https://rdrr.io/r/base/ns-load.html);
- source script preserved under `inst/extdata/source-scripts/` for
  provenance.

## Release note

The maintainer email in `DESCRIPTION` remains an explicit `.invalid`
placeholder and must be replaced with a verified address before CRAN
submission.
